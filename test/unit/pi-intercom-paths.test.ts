...
test("getBrokerSocketPath includes uid scope while preserving the legacy digest", () => {
  ...
  const uid501 = getBrokerSocketPath(agentDir, "/tmp", 501);
  const uid502 = getBrokerSocketPath(agentDir, "/tmp", 502);
...
test("getBrokerSocketPath falls back to /tmp only when the preferred socket path is too long", () => {
  const socketPath = getBrokerSocketPath(`/agent-${randomUUID()}`, `/private/${"long/".repeat(30)}tmp`);
  assert.match(socketPath, /^\/tmp\/pi-intercom-[a-f0-9]{16}\/broker\.sock$/);
});

test("prepareBrokerSocketPath rejects an unusable socket directory at runtime", () => {
  ...
  try {
    assert.throws(() => prepareBrokerSocketPath(agentDir));
  } finally {
...
test("prepareBrokerSocketPath never switches startup ownership to a live legacy socket", async () => {
  const agentDir = `/tmp/pi-agent-${randomUUID()}`;
  const legacyPath = getLegacyBrokerSocketPath(agentDir);
  ...
  await once(server, "listening");
  try {
    assert.equal(prepareBrokerSocketPath(agentDir), getBrokerSocketPath(agentDir));
