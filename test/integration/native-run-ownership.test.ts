...
it("native workflow outcomes retain successful evidence, truthful grouped notifications, and actual children across reload in both modes", { timeout: 120_000 }, () => {
	const root = fs.mkdtempSync(path.join(os.tmpdir(), "pi-native-workflow-outcomes-"));
	...
	try {
		...
		if (process.env.PI_OWNERSHIP_KEEP_EVIDENCE) console.log(`Native workflow evidence: ${root}/workflow-evidence.json`);
