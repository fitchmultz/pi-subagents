...
try {
  const args = fork ? ["scripts/run-tests.mjs", "integration"] : ["--test", "--test-concurrency=2", ...contracts.map(name => `test/integration/${name}.test.ts`)];
  const result = spawnSync(process.execPath, args, { env, stdio: "inherit", timeout: 360_000 });
