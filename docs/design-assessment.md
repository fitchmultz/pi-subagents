...
**Shared workflow decisions:** foreground and detached hosts use `runParallelTasks` over the existing `mapConcurrent`, and `completeWorkflowStep` for sequential, static-parallel, and dynamic completion. Hosts still own child processes, live controls, status files, presentation, and worktree cleanup. A failed or stopped group retains completed siblings’ named outputs and evidence but cannot advance. Dynamic collections publish only after every child and the collection schema succeed; a successful prefix is not a completed workflow. Fail-fast carries its own abort reason instead of masquerading as a user pause.

**Preparation parity:** both modes resolve task defaults with `resolveChainTemplates` and render task, previous-output, named-output, and item substitutions in one literal pass. Dynamic runner validation preserves child resource limits. Empty expansions retain their group graph node without consuming an executed-child slot; child indices and intercom targets remain aligned, while reserved fork-session slots keep their original associations.
