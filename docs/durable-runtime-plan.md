...
- Put unanswered questions and failures first, then live runs, then completed-but-unreviewed work. Parent review is explicit and separate from execution, validation, and notification. Inspection, review, and a late nudge do not restart work.
- Share retry, acceptance finalization, and workflow decisions across the concrete foreground and detached hosts. Preserve successful sibling outputs as evidence when a group fails; do not run downstream steps after failed, paused, or detached work. Publish a dynamic collection only after its children and collection schema succeed. A detached child finishing does not complete a chain with unstarted steps or an unpublished collection.
- Show compact TUI summaries for delegation receipts, subagent results, and completion messages. Native expansion reveals the full response; model-visible instructions and results remain unchanged.
...
Official released Pi **v0.85.1** supports ordinary extension use: profile discovery, tool activation, ordinary Intercom delivery, acceptance-contract launches, and same-directory saved-child resumes. Published-package verification establishes that baseline. Historical results on unreleased upstream source `e4ce7b449` remain separate evidence; they do not establish testing at the release commit.

The stronger durable-runtime guarantees in this plan require the corrected native `fitchmultz/pi` build documented under [Installation](../README.md#installation), including prompt-admission ownership, settlement, custom-queue reporting, the restart notice, and `--session-cwd`. Published 0.85.1 retains the documented cancellation/queued-message, busy-user-preparation, custom-queue visibility, and idle-wakeup guidance limitations. Saved-child cwd overrides also require native `--session-cwd` when the requested directory differs or the header cannot establish the same physical directory. Stock Pi 0.84.x likewise lacks the stronger contracts. The corrected fork also reports 0.85.1; development pins and version output do not identify its fixes.
...
Release qualification includes ordinary released-Pi support and the additional fork guarantees. It requires parent review, the full macOS/Linux package gates against the corrected fork build, and isolated model-backed smoke tests. The full gate retains stronger native regressions that are expected to fail on published 0.85.1; ordinary support does not imply full runtime parity. The Linux gate takes a credential-free prebuilt native archive and keeps every package test; see [local validation](../README.md#local-validation) for the archive shape and all four SDK overrides.

Keep installed code and active sessions untouched until delivery is approved. Checkpoint and fully quit every Pi session using the affected installation, then wait at least five seconds after the last broker disconnect. Use a separate terminal for updates. If qualifying the stronger fork guarantees, verify the native installation contains the companion fix and rebuild it only if missing. Then update this package before reopening Pi. Resume the same saved parent session before testing active tools; a new or forked parent must not adopt the original parent's runs or pending intercom delivery.
