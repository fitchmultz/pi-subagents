...
Full support requires the corrected native `fitchmultz/pi` build documented under [Installation](../README.md#installation), including prompt-admission ownership, settlement, custom-queue reporting, the restart notice, and `--session-cwd`. Stock Pi 0.84.x and published 0.85.1 lack these contracts. The corrected build also reports 0.85.1; development pins and version output do not identify the fix.
