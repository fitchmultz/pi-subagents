...
function subagentCallKey(toolName: unknown, args: unknown): string | undefined {
	if ((toolName !== "subagent" && toolName !== "delegate" && toolName !== "agent_runs") || !isRecord(args)) return undefined;
	try {
...
export function recordToolStartForSubagentLoopGuard(input: {
	...
	if (listCount >= REPEATED_SUBAGENT_CALL_LIMIT) {
		return `Child appears stuck repeating ${input.toolName}({ action: "${args.action}" }) ${listCount} times. Stopping to avoid a tool loop.`;
	}
	return undefined;
}
...
export function recordToolEndForSubagentLoopGuard(input: {
	...
	const entry = [...input.state.recentSubagentCalls].reverse().find((candidate) =>
		candidate.toolName === input.toolName && candidate.failed === undefined
		&& (!toolCallId || !candidate.toolCallId || candidate.toolCallId === toolCallId));
	...
	if (failedCount >= REPEATED_SUBAGENT_CALL_LIMIT) {
		return `Child appears stuck repeating the same failed ${input.toolName} call ${failedCount} times. Stopping to avoid a tool loop.`;
	}
	return undefined;
}
