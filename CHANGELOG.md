...
### Changed
- Sync bundled role routing with the maintainer's active profiles. Every role except `reviewer-claude` and `delegate` now prefers GPT-6 Astra through a ChatGPT/Codex subscription (`openai-codex`) at medium thinking, falling back to the OpenAI API route, so either account works. `reviewer-claude` uses Claude Opus 5.5 at high thinking, falling back to Fable 5.1 and then the matching Cloudflare AI Gateway routes. Routes that no longer ship (Grok 4.6, GPT-5.6 Sol, Kimi K3) are removed from the defaults.
...
- Interruptible `async: false` continuation/answer execution that follows the actual run identity and preserves saved launch choices. Important steers release foreground execution while children keep working and deliver completion automatically.
...
- Open completed agent conversations on the readable saved report instead of serialized submission data, without repeating an existing native answer. Group tools with Pi's native renderers while retaining full recorded details, contextual replies, and saved reading positions. Show recorded custom-tool diffs readably in explicit Details before raw metadata.
- Frame and pad the Agents picker and conversation in both terminal modes. Give task labels the available width, filter across full assignments, and keep the original assignment directly inspectable.
...
- Correct local runtime install/rebuild guidance to use the existing `npm install --omit=dev` lifecycle, avoiding development Pi copies without claiming a loader fix.
...
- Require the requested workflow to finish before reporting completion, and retain workflow-level failures in saved results and completion notices even when every launched child succeeds. Detached chains remain paused after their children finish when downstream steps or collection publication remain unfinished. Publish dynamic collections only after every child and the collection schema succeed.
- Align omitted task defaults, literal template substitution, previous-output handoffs, and dynamic child resource limits across both modes. Empty fanouts retain correct child counts and logical progress across restart; preallocated fork sessions retain their associations across later expansions.
...
- Continue with the actual saved provider/model, thinking, profile and output/acceptance choices instead of rediscovering changed defaults. Snapshot each finished child's choices before a continuation changes the shared session. Recover older native receipts where possible and require an explicit profile choice when the original profile was not saved.
...
- Route async completions to the resumed parent's current intercom address. Keep fallback delivery quiet in the TUI while retaining the saved failure diagnosis.
...
- Deliver async result events before completion cleanup, enforce `maxOutput` on each delivered child result, snapshot detached foreground returns, keep nested detached runs live until their terminal event, inject acknowledged foreground grouped results back into the active orchestrator before it can continue without the child output, keep periodic fallback scanning available for orphaned completion files after reloads, debounce status writes, clear recovered attention notifications, and harden stale status, nested control, and parallel-group normalization.
- Preserve builtin override precedence and effective last-definition-wins discovery, refuse ambiguous management mutations when duplicate definitions exist in one scope, keep managed JSON chain skill fields round-trippable, accept whitespace after frontmatter delimiters, and surface bounded invalid agent, chain, frontmatter, slash-command, and config diagnostics.
