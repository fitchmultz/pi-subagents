...
| `context-builder` | Requirements/codebase handoff builder | `cloudflare-ai-gateway/gpt-5.6-sol` | Writes structured context and meta-prompts |
...
| `planner` | Creates implementation plans | `cloudflare-ai-gateway/gpt-5.6-sol` | Writes `plan.md` |
| `worker` | Bounded implementation | `cloudflare-ai-gateway/gpt-5.6-sol` | Single-writer implementation and validation |
| `debugger` | Root-cause diagnosis | `cloudflare-ai-gateway/gpt-5.6-sol` | Writes `diagnosis.md` |
| `fixer` | Decided, bounded remediation | `cloudflare-ai-gateway/gpt-5.6-sol` | Applies an explicit fix list |
| `reviewer` | General implementation review | `cloudflare-ai-gateway/gpt-5.6-sol` | Review-only by default |
| `reviewer-gpt` | Strict completion gate | `cloudflare-ai-gateway/gpt-5.6-sol` | Maintainability/correctness review |
| `reviewer-claude` | Cross-model product-risk review | `cloudflare-ai-gateway/gpt-5.6-sol` | Independent review |
| `reviewer-security` | Trust-boundary review | `cloudflare-ai-gateway/gpt-5.6-sol` | Security/data-safety findings |
| `reviewer-ponytail` | Over-engineering and slop review | `cloudflare-ai-gateway/gpt-5.6-sol` | Deletion-focused findings; behavior-preserving only |
| `ui-designer` | UI and accessibility review | `cloudflare-ai-gateway/gpt-5.6-sol` | Rendered UX guidance |
| `writer` | Human-facing writing | `cloudflare-ai-gateway/gpt-5.6-sol` | Writes `draft.md` |
| `oracle` | Decision-consistency advisory review | `cloudflare-ai-gateway/gpt-5.6-sol` | Forked advisory review |
| `delegate` | Lightweight generic delegate | `cloudflare-ai-gateway/gpt-5.6-sol` | No fixed output; generic delegated work |

Every Fitch role profile falls back to `openai/gpt-5.6-sol`, then `openai-codex/gpt-5.6-sol`. Keep those configured defaults unless a run, user setting, or project setting has a concrete reason to override them.
