...
Children also receive `PI_SUBAGENT_ROOT_SESSION_ID`: the owning root's actual Pi session ID, inherited unchanged through fresh/forked children, nested delegation, detached runner configuration, and saved launch/revive records. This is independent of cwd, transcript file paths, run IDs, and ordinary Pi fork/clone ancestry. Extensions such as pi-agent-browser-native use it to share one browser within the parent/descendant group while keeping unrelated roots independent. The parent process environment is not changed. Resume through the owning parent's existing continuation path to retain saved root identity; opening a child transcript as an unrelated standalone root does not adopt that group.
