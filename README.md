...
`npm run check:compat` uses the selected host installed in this checkout, never a hidden Pi from PATH. Every lane checks host SDK/manifest-bin identity, builds, typechecks, and qualifies both compiled entries with a private Intercom broker through the native SDK and bundled RPC CLI. Official hosts exercise same/different-cwd resume, acceptance, structured output, native result routing/ownership and tool activation. No provider credentials or inference services are used.

CI runs all unit tests and the packed-consumer/local-install checks once, on Linux Node 24 with the fork. The complete integration suite is split with Node's native `--test-shard` across the three fork lanes: Linux Node 22.19 (1/3), Linux Node 24 (2/3), and macOS Node 24 (3/3). Each integration file runs once; the three lanes together qualify the full suite. Every lane also retains the shared automation's fresh Git-consumer installation and bundled CLI checks. Outside CI, `check:compat` runs unit and package checks plus the selected host's unsharded contracts.

The compatibility runner supplies `PI_COMPAT_HOST=official|fork`, `PI_COMPAT_EXPECTED_VERSION`, `PI_COMPAT_EXPECTED_PACKAGE_DIR`, `PI_HOST_INDEX`, and `PI_HOST_CLI`. Types, SDK imports, and child CLI must resolve to that installed graph. `PI_COMPAT_HOST=fork` requires native checkpoint hooks instead of silently skipping them; CI distributes its integration suite as described above. The ordinary official lane does **not** certify the extended replay/working-session contract: full official 0.87.0 integration still exposes five unchanged queue visibility, prompt-preparation ownership/startup, and `newContext` failures. The existing portable/replay fork baseline is [`fitchmultz/pi` at `afed789dded723566b6ecb1c77a06e8561504f7a`](https://github.com/fitchmultz/pi/commit/afed789dded723566b6ecb1c77a06e8561504f7a) (Pi 0.87.0). Native async and immediate accounting are qualified separately on the [enhanced host](#host-capabilities-and-result-delivery). See [host limitations](docs/intercom.md#limitations).
