...
- For a non-terminal material change, use `contact_supervisor` with `reason: "progress_update"` when available. Include the new state, prior state, timestamp, and concise evidence such as a URL or run identifier. The update steers at the next tool boundary. Skip starts and redundant status; retain material findings in the final result.
